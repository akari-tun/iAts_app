package com.tun.app.iats_app.bluetooth;
...
public class BT_New extends Communication {
	...
	private static final UUID SerialPortServiceClass_UUID = UUID.fromString("00001101-0000-1000-8000-00805F9B34FA");
	...
	SimpleQueue<Integer> fifo = new SimpleQueue<>();
	...
	public synchronized void stop() {
		...
		sendMessageToUI_Toast(context.getString(R.string.info_disconnected));
	}
	...
	private void connectionFailed() {
		...
		sendMessageToUI_Toast(context.getString(R.string.error_bluetooth_not_support));
	}
	...
	private void connectionLost() {
		...
		sendMessageToUI_Toast(context.getString(R.string.info_unable_to_connect));
	}
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					mHandler.obtainMessage(MESSAGE_READ, bytes, -1, current).sendToTarget();

					// String a = buffer.toString();
					Log.d(TAG, "read data <- " + new String(current));
		...
		public void write(byte[] buffer) {
			try {
				...
				Log.d(TAG, "write data -> " + new String(buffer));
	...
	public BT_New(Context context) {
		...
		mAdapter = getDefaultAdapter(context);
		if (mAdapter == null) {
	...
	public static BluetoothAdapter getDefaultAdapter(Context context) {
		...
		try {
			...
			Log.e(TAG, "Bluetooth is can not use", e);
		}
